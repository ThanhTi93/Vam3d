      ...
      {showPlayer ? (
        <div className="flex flex-col">
          <div className="relative bg-black aspect-video w-full flex items-center justify-center">
            {isCurrentRoute ? (() => {
              const currentEp = movie.episodes?.[activeEpisode];
              const videoSrc = currentEp?.url || movie.videoUrl;
              const storageKey = `progress_movie_${movie.id}_ep_${activeEpisode}`;

              const handleEpisodeEnd = () => {
                if (movie.episodes && activeEpisode < movie.episodes.length - 1) {
              ...
              return (
                <VideoPlayer
                  src={videoSrc}
                  bunnyVideoId={currentEp?.bunnyVideoId}
                  bunnyStatus={currentEp?.bunnyStatus}
                  poster={currentEp?.banner || movie.bannerUrl || movie.imgUrl}
                  title={`${movie.title} - ${currentEp?.name || "Tập phim"}`}
